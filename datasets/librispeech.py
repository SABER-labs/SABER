...
def convert_to_mel(signal):
    ...
    transforms = Compose([FixAudioLength(config.max_audio_length_in_secs), to_stft, convert_to_mel, DeleteSTFT(), ToAudioTensor(['mel_spectrogram'])])
    return transforms(data)

def image_train_transform(spec, epoch=config.epochs, unlabelled=False):
    ...
    random_apply_specaugment = RandomApply([RandomChoice([SpecAugmentOnMel(no_time_wrap=unlabelled), SpecSprinkleOnMel()])], p=frac_to_apply)
...
def allign_collate(batch, device='cpu'):
    ...
    epoch = epochs[0]
    imgs = torch.stack([image_train_transform(img, epoch) for img in img_list])
    ...
    return (imgs.squeeze(1), labels, label_lengths)
...
def align_collate_unlabelled(batch, device='cpu'):
    ...
    return (imgs.squeeze(1), augmented_imgs.squeeze(1))
