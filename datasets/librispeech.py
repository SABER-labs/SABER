...
def image_train_transform(spec, epoch):
    ...
    frac_to_apply = np.clip(epoch / config.augment_warmup_epoch, 0, 0.9)
    ...
    random_apply_specaugment = RandomApply([RandomChoice([SpecAugmentOnMel(), SpecSprinkleOnMel(), SpecNoise()])], p=frac_to_apply)
