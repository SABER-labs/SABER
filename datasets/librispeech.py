...
def convert_to_mel(signal):
    ...
    transforms = Compose(
        [to_stft, stft_to_mel, DeleteSTFT(), ToAudioTensor(['mel_spectrogram'])])
...
def image_train_transform(spec, epoch):
    ...
    frac_to_apply = np.clip(epoch / config.augment_warmup_epoch, 0.05, 0.85)
...
def image_val_transform(spec):
    ...
    transforms = Compose(
        [ToAudioTensor(['mel_spectrogram']), torch.from_numpy])
    return transforms(data).unsqueeze(0)
...
def allign_collate(batch, device='cpu'):
    ...
    epoch = epochs[0]
    imgs = torch.nn.utils.rnn.pad_sequence([image_train_transform(img, epoch=epoch).squeeze(
        0).permute(1, 0) for img in img_list], batch_first=True, padding_value=0).permute(0, 2, 1)
    flat_label_list = np.concatenate(label_list).astype(np.int32)
    labels = torch.from_numpy(flat_label_list)
    label_lengths = torch.tensor([label.shape[0]
                                  for label in label_list], dtype=torch.int32)
    return (imgs, labels, label_lengths)


def allign_collate_val(batch, device='cpu'):
    img_list, label_list, _ = zip(*batch)
    imgs = torch.nn.utils.rnn.pad_sequence([image_val_transform(img).squeeze(0).permute(
        1, 0) for img in img_list], batch_first=True, padding_value=0).permute(0, 2, 1)
    flat_label_list = np.concatenate(label_list).astype(np.int32)
    labels = torch.from_numpy(flat_label_list)
    label_lengths = torch.tensor([label.shape[0]
                                  for label in label_list], dtype=torch.int32)
...
def align_collate_unlabelled(batch, device='cpu'):
    ...
    imgs = torch.nn.utils.rnn.pad_sequence([image_val_transform(img).squeeze(0).permute(
        1, 0) for img in img_list], batch_first=True, padding_value=0).permute(0, 2, 1)
    augmented_imgs = torch.nn.utils.rnn.pad_sequence([image_train_transform(img, epoch=config.augment_warmup_epoch).squeeze(
        0).permute(1, 0) for img in img_list], batch_first=True, padding_value=0).permute(0, 2, 1)
