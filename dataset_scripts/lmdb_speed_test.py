...
from datasets.librispeech import allign_collate, label_re_transform
import os
...
def test_data(loader, name, epoch):
    max_length = 0
    start = time.time()
    for i, (img, label, label_lengths) in enumerate(tqdm(loader)):
...
if __name__ == '__main__':
    ...
    for epoch in range(1):
        data_loader_labbeled = torch.utils.data.DataLoader(data_labbeled, batch_size=train_batch_size, shuffle=True, num_workers=30, pin_memory=False, collate_fn=partial(allign_collate, epoch=epoch))
        data_loader_unlabbeled = torch.utils.data.DataLoader(data_unlabbeled, batch_size=train_batch_size, shuffle=True, num_workers=30, pin_memory=False, collate_fn=partial(allign_collate, epoch=epoch))
