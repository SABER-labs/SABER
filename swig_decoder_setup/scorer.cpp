...
Scorer::Scorer(double alpha,
  ...
  sentencepiece_model_ = nullptr;

  max_order_ = 0;
  dict_size_ = 0;
  SPACE_ID_ = -1;

  setup(lm_path, sentencepiece_path, vocab_list);
}
...
Scorer::~Scorer() {
  ...
  if (sentencepiece_model_ != nullptr) {
    delete static_cast<sentencepiece::SentencePieceProcessor*>(sentencepiece_model_);
  }
...
void Scorer::load_sp(const std::string &sentencepiece_path) {
  VALID_CHECK_EQ(access(sentencepiece_path.c_str(), F_OK), 0, "Invalid sentencepiece model path");
  const auto filename_sp = sentencepiece::util::min_string_view(sentencepiece_path);
  sentencepiece::SentencePieceProcessor sp;
  const auto status = sp.Load(filename_sp);
  if (!status.ok()) {
   std::cerr << status.ToString() << std::endl;
  } else {
    sentencepiece_model_ = &sp;
  }
}
...
std::string Scorer::vec2str(const std::vector<int> &input) {
  std::string word = "i have no idea what am i debugging";
  sentencepiece::SentencePieceProcessor* sp_model = static_cast<sentencepiece::SentencePieceProcessor*>(sentencepiece_model_);
  const int piece_size = sp_model->GetPieceSize();
  // const auto status_dec = sp_model->Decode(input, &word);
  // if (!status_dec.ok()) {
  //  std::cerr << status_dec.ToString() << std::endl;
  // }
  return word;
}
